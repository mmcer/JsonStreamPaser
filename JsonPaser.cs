...
namespace JsonStreamPaser
{
    ...
    {
        const int STATE_START_DOCUMENT = 0;
        const int STATE_DONE = -1;
        const int STATE_IN_ARRAY = 1;
        const int STATE_IN_OBJECT = 2;
        const int STATE_END_KEY = 3;
        const int STATE_AFTER_KEY = 4;
        const int STATE_IN_STRING = 5;
        const int STATE_START_ESCAPE = 6;
        const int STATE_UNICODE = 7;
        const int STATE_IN_NUMBER = 8;
        const int STATE_IN_TRUE = 9;
        const int STATE_IN_FALSE = 10;
        const int STATE_IN_NULL = 11;
        const int STATE_AFTER_VALUE = 12;
        const int STATE_UNICODE_SURROGATE = 13;

        const int STACK_OBJECT = 0;
        const int STACK_ARRAY = 1;
        const int STACK_KEY = 2;
        const int STACK_STRING = 3;

        const int BUFFER_MAX_LENGTH = int.MaxValue;

        int state;
        int[] stack = new int[20];
        int stackPos = 0;

        private JsonListener myListener;

        bool doEmitWhitespace = false;
        // fixed length buffer array to prepare for c code
        char[] buffer = new char[char.MaxValue];//[BUFFER_MAX_LENGTH];
        int bufferPos = 0;

        char[] unicodeEscapeBuffer = new char[10];
        int unicodeEscapeBufferPos = 0;

        char[] unicodeBuffer = new char[10];
        ...
        public JsonPaser(JsonListener listener):this()
        {
            myListener = listener;
        }
        ...
        {
            bufferPos = Math.Min(bufferPos + 1, BUFFER_MAX_LENGTH - 1);
        }
        ...
        void endString()
        {
            ...
            {
                buffer[bufferPos] = '\0';
                myListener?.Key(new string(buffer));
                state = STATE_END_KEY;
            }
            else if (popped == STACK_STRING)
            {
                buffer[bufferPos] = '\0';
                myListener?.Value(new string(buffer));
